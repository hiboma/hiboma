...
Linuxのファイルシステムはカーネルモジュールとして実装できる

## モジュールの init で register_filesystem を呼びファイルシステムを登録する
 
俺俺のファイルシステムを登録するにあたって struct file_system_type を定義する

## struct file_system_type の定義

super_block のコンストラクタ/デストラクタを実装するのが目的

 * fils_system_type の .get_sb と .kill_sb フィールドが要る
   * ひな形として流用できる関数が用意しされているので利用する
 * .get_sb は get_sb_nodev をラップすればよい
   * デバイスを持たないファイルシステムで superblock を取得するための関数 [#](http://filesystem.g.hatena.ne.jp/n314/20080325/1206426881)
    * get_sb_nodev の中で sget が superblock *sb のメモリを割り当ててくれる
	     * fill_super関数ポインタを実装して superblock のフィールドを初期化する必要がある [>][fill_super]
   * kill_sb は kill_anon_super/kill_litter_super を使えばよい 

__misc__

 * .get_sb フィールドは 2.6.39 まで。それ以降は .mount になる (vboxfsを参照)
   * get_sb_nodev が変わり mount_nodev になる
 * kill_litter_super [#](http://lxr.free-electrons.com/source/fs/super.c?v=2.6.32#L703) は kill_anon_super のラッパーでd_genocide [#](http://lxr.free-electrons.com/source/fs/dcache.c?v=2.6.32#L2176) を呼ぶ
   * d_genocide は root dentry から辿って全てのサブディレクトリ以下の dentry の参照カウントをデクリメントする
   * kill_anon_super の ___anon___ は super_block と結びついた block deviceが無いことを指すのだろう (anonymouse page 的な)

## superblock の初期化 fill_super の役割

 * ファイルシステムの superblock と、ファイルシステムの root ディレクトリとなる inode/dentry の初期化をする必要がある (後述)
 * ***_fill_super で実装して get_sb_nodev に関数ポインタとして渡す形式
   * vboxsf は sf_read_super_auxで実装している 
   * tmpfs は shmem_fill_super で実装している [#](http://lxr.free-electrons.com/source/mm/shmem.c?v=2.6.32#L2305)
...
sb は get_sb_nodev [#](http://lxr.free-electrons.com/source/fs/super.c?v=2.6.32#L855) の中で sget [#](http://lxr.free-electrons.com/source/fs/super.c?v=2.6.32#L338) -> alloc_super [#](http://lxr.free-electrons.com/source/fs/super.c?v=2.6.32#L54 )で割り当てられたもので、下記の各種フィールドが初期化されていない
 ...
 * root になる dentry 
  * rootディレクトリの inode を作り、さらに d_alloc_root で root dentry 化する必要がある [>][inode]
    * tmpfs の場合 shmem_get_inode [#](http://lxr.free-electrons.com/source/mm/shmem.c?v=2.6.32#L1541) の `new_inode(sb)` から inode を作る
