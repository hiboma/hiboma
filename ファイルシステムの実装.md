 ...
 * .mkdir は自前で頑張る。 .rmdir は [simple_rmdir](http://lxr.free-electrons.com/ident?v=2.6.32&i=simple_rmdir) で代用できた
 * simple_rmdir は [simple_unlink](http://lxr.free-electrons.com/source/fs/libfs.c?v=2.6.32#L287) 呼び出している
 * 通常ファイルと同じように unlink するけど、inode->i_nlink(リンク数) を drop_nlink デクリメントするのが違う
