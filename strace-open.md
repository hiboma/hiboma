...
open(2) を呼ぶプロセスを strace すると、下記のように人間に読みやすい表示で出力されます

```c
open("./hello.txt", O_RDWR|O_CREAT|O_TRUNC, 0666) = 3
```

この表示がどのように処理されて作られているのか (一部) 追ってみます。 strace のバージョンは 4.10 です

strace 自体の仕組みは説明しません。ptrace(2) を使って実現していると思われますが、複雑なのでここでの解説は無理です
