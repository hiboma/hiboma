...
🤖 注目すべき syscall を diff 記法で強調すると以下の通りです。
...
> [!NOTE]
> `rename(2)` はディレクトリエントリの操作であり、inode への書き込みではないため ETXTBSY を返しません。実行中バイナリの安全な更新にはこの手法を使います。

### 🤖 cp -f / cp --remove-destination / rename の動作比較

> 🤖 This section was generated by AI.

#### cp -f: open 失敗後に unlink してリトライ

```mermaid
sequenceDiagram
    participant P as cp -f
    participant K as Kernel
    participant I as inode (実行中)

    P->>K: open(".sleep", O_WRONLY|O_TRUNC)
    K-->>P: -ETXTBSY
    Note over P: open 失敗を検知
    P->>K: unlink(".sleep")
    K->>I: ディレクトリエントリ削除<br>(inode は実行中プロセスが参照し残存)
    K-->>P: 0 (成功)
    P->>K: open(".sleep", O_CREAT|O_EXCL)
    Note over K: 新しい inode を作成
    K-->>P: fd=4 (成功)
    P->>K: write(fd, ...)
    K-->>P: 成功
```

#### cp --remove-destination: open 前に無条件で unlink

```mermaid
sequenceDiagram
    participant P as cp --remove-destination
    participant K as Kernel
    participant I as inode (実行中)
...
#### rename: テンポラリファイル経由で atomic に置き換え

```mermaid
sequenceDiagram
    participant P as rsync
    participant K as Kernel
    participant I as inode (実行中)

    P->>K: open(".sleep.XXXXXX", O_CREAT|O_WRONLY)
    Note over K: テンポラリファイル用の<br>新しい inode を作成
    K-->>P: fd=4 (成功)
    P->>K: write(fd, ...)
    K-->>P: 成功
    P->>K: rename(".sleep.XXXXXX", ".sleep")
    K->>I: 旧ディレクトリエントリを新 inode に付け替え<br>(旧 inode は実行中プロセスが参照し残存)
    Note over K: atomic な操作。<br>".sleep" が存在しない瞬間がない
    K-->>P: 0 (成功)
```

> [!WARNING]
> `cp -f` と `cp --remove-destination` は unlink から open/write 完了までの間、ファイルが存在しない瞬間が生じます (非 atomic)。この間に別プロセスがファイルを参照すると `ENOENT` になります。`rename(2)` にはこの問題がありません。
...
```mermaid
stateDiagram-v2
    state "i_writecount = 0\n(初期状態)" as zero
    state "i_writecount > 0\n(書き込み中)" as positive
    state "i_writecount < 0\n(実行中)" as negative

    [*] --> zero
    zero --> positive: get_write_access()\nopen(O_WRONLY) 成功
    positive --> zero: put_write_access()\nclose() / fput()
    zero --> negative: deny_write_access()\nexecve() 成功
    negative --> zero: allow_write_access()\nプロセス終了

    positive --> positive: deny_write_access() → ETXTBSY\n(execve 拒否)
    negative --> negative: get_write_access() → ETXTBSY\n(open(O_WRONLY) 拒否)
```
...
> [!WARNING]
> この設計には潜在的リスクがあります。まだ読み込まれていないページ（デマンドページング対象）については、ファイル書き換え後に vnode pager 経由で新しいデータが読まれる可能性があります [^demand-paging-risk]。Apple はコード署名 (`cs_validate_range()`)、SIP、Gatekeeper という多層防御でこのリスクを緩和しています。
